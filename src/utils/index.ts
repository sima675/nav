...
export function queryString(): {
  ...
  const parseQs:any = qs.parse(search)
  const parseQsPage:string = parseQs.page
  const parseQsId:string = parseQs.id
  let id = parseInt(parseQsId) || 0
  let page = parseInt(parseQsPage) || 0
